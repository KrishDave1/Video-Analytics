...
import tempfile
import torch
# import base64  # No longer needed since we removed imageData
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None, global_w=1000, global_h=1800, model_type='vit_b', keep_input_size=True, max_size=1080):
        ...
        fitToWindow = action(
            ...
            lambda: self.rescaleCurrentImage(),
            'f',
            "view",
            self.tr("Fit current image to window"),
            enabled=True,
        )
        ...
        self.toolbar.addAction(fitToWindow)
    ...
    def saveLabels(self, filename):
        ...
        def format_shape(s):
            # Create a minimal shape data structure
            data = {
                "label": s.label.encode("utf-8") if PY2 else s.label,
                "points": [[p.x(), p.y()] for p in s.points],
                "shape_type": s.shape_type,
            }
            # Only include group_id if it's not None/empty
            if s.group_id:
                data["group_id"] = s.group_id
        ...
        # Use only the filename instead of full path to reduce JSON size
        image_filename = os.path.basename(self.current_img) if self.current_img else ""
        
        save_data = {
            "version": "1.0.0",
            "flags": {},
            "shapes": shapes,
            "imagePath": image_filename,  # Only filename, not full path
            "imageHeight": self.raw_h,
            "imageWidth": self.raw_w
            # Removed imageData to significantly reduce file size
        }

        with open(filename, 'w') as f:
            json.dump(save_data, f, separators=(',', ':'))  # Compact JSON without spaces
        return True
    ...
    def loadImg(self):
        ...
        # Auto-scale image to fit the window while maintaining aspect ratio
        window_width = self.scrollArea.width() - 20  # Leave some margin
        window_height = self.scrollArea.height() - 20
        
        # Calculate scaling factor to fit the image in the window
        scale_w = window_width / pixmap.width()
        scale_h = window_height / pixmap.height()
        ...
        self.original_pixmap = QPixmap(self.current_img)

    def resizeEvent(self, event):
        """Handle window resize events to rescale the current image"""
        super().resizeEvent(event)
        if hasattr(self, 'original_pixmap') and self.original_pixmap and not self.original_pixmap.isNull():
            self.rescaleCurrentImage()

    def rescaleCurrentImage(self):
        """Rescale the current image to fit the window"""
        if not hasattr(self, 'original_pixmap') or not self.original_pixmap or self.original_pixmap.isNull():
            return
